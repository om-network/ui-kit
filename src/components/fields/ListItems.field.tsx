...
export const ListItemsFieldToolbar = ({ children, last, index, onAdd, onRemove }: ListItemsFieldToolbarProps & ListItemsFieldDispatch) => (
    <div className="flex justify-center items-center">
        ...
        <label htmlFor="action" className="ml-2">
            <span>
                ...
                {!last && <Hi.MinusCircleIcon className="h-6 w-6 text-red-800 hover:text-red-600 cursor-pointer" onClick={() => onRemove && onRemove(index)} />}
            </span>
        </label>
    </div>
);
...
export const ListItemsField = (props: ListItemsFieldProps & ListItemsFieldDispatch) => {
    const { formRef, label, subLabel, name, max = 5, wrapperClasses, children, collapsed = false, canCollapse = false } = props;
